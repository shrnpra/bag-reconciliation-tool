...
export default function CheckInForm() {
  ...
  const [success, setSuccess] = useState<{ driverInventory: number; storeInventory: number; hasDiscrepancy: boolean } | null>(null)
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true)
    try {
      const { data } = await axios.post('/api/visits/checkin', {
        driverId: user!.id,
        storeId,
        bagCount,
      })
      setSuccess({ driverInventory: data.driverInventory, storeInventory: data.storeInventory, hasDiscrepancy: data.hasDiscrepancy })
    } catch (err: any) {
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-8">
      <h1 className="mb-6 text-xl font-bold text-gray-800">Bag Check-In</h1>
      <form onSubmit={handleSubmit} className="space-y-4 rounded-xl bg-white p-6 shadow">
        ...
        <div>
          <label htmlFor="bagCount" className="mb-1 block text-sm font-medium text-gray-700">Bag Count (1–999)</label>
          <input id="bagCount" type="number" min={1} max={999} value={bagCount} onChange={e => setBagCount(Number(e.target.value))} required
            className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
        </div>
        {error && <p role="alert" className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">{error}</p>}
        {success && success.hasDiscrepancy && (
          <div className="rounded-lg bg-yellow-50 border border-yellow-200 px-3 py-2 text-sm text-yellow-800">
            <p className="font-medium">⚠️ Discrepancy detected</p>
            <p>This visit has been flagged for review.</p>
